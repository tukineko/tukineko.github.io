...
var GameLayer2 = cc.Layer.extend({
	...
	ctor:function () {
		...
		//初期化
		this._game_state = GameLayer2.GameState["READY"];
		this._charas = [];
		this._enemies = [];
		this._score = 0;
		this._dx = 10;
		...
		cc.eventManager.addListener({
			...
			onTouchMoved: function(touch, event) {
				if (this._game_state === GameLayer2.GameState["PLAYING"]) {
	...
	onEnterTransitionDidFinish: function() {
		this._super();
		this.onGameStart();
		//this._game_state = GameLayer2.GameState["PLAYING"];
	},
	//スコア表示処理
	...
	//ゲームオーバー処理
	onGameOver: function(){
		this._game_state = GameLayer2.GameState["ENDING"];
		...
		gameover.runAction(
			cc.sequence(
				...
				cc.callFunc(function() {
					this._game_state = GameLayer2.GameState["RESULT"];
					this.onResult();
				}, this)
			)
		);
